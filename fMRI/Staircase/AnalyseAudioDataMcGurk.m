function AnalyseAudioDataMcGurk
...
for i=1:NbMcMovies
	...
	RespTypeRecap{i,1} = McMoviesDirList(i).name(1:end-4);
    RespTypeRecap{i,2} = zeros(7,length(NoiseSoundRange(i,:)));
		
	MoviesLists = [MoviesLists ; McMoviesDirList(i).name(1:end-4)];
end
...
set(gca,'tickdir', 'out', 'xtick', [20] ,'xticklabel', 'McGurk', 'ticklength', [0.002 0], 'fontsize', 13)
axis([0 40 0 3])
...
for i=1:NbTrials
	...
	if  TotalTrials{1,1}(i,4)>.4 & TotalTrials{1,1}(i,5)~=666
		...
		if TotalTrials{1,1}(i,2)==2
			...
			A = find( strcmp (cellstr( repmat(TotalTrials{2,1}(i,:), NbMcMovies, 1) ), cellstr(MoviesLists) ) );
...
Missed = length( [find(TotalTrials{1,1}(:,5)==666)' find(TotalTrials{1,1}(:,4)<0.4)'] ) / length (TotalTrials{1,1}(:,4))
