    ...
    for h=1:length(SubjectsList)
        ...
        for Day_Ind=1:2
            if exist(['DenoiseDay' num2str(Day_Ind) '.mat'], 'file') 
                ...
                if exist('DenoiseResults', 'var') 
                    if length(Runs2Include)>1
                        for RunInd=1:length(Runs2Include)
                            NbNoisRegressors(Runs2Include(RunInd)) = DenoiseResults.pcnum;
                        end
                    elseif length(Runs2Include)==1
                        NbNoisRegressors(Runs2Include) = 1;
                    else
                        NbNoisRegressors(Runs2Include) = [];
                    end
                end
        ...
        %%
        if any(NbNoisRegressors)
            PC_Regressor = [];
            RegBefPC = 2*sum(X)+2;
            RegPerSess = RegBefPC+NbNoisRegressors;
            RegPrevSess = [0 RegPerSess(1:end-1)];

            for j=1:size(RegBefPC,2)
                TEMP = [];
                TEMP = [zeros(NbNoisRegressors(j),sum(RegPrevSess(1:j))) ...
                    zeros(NbNoisRegressors(j),RegBefPC(j)) ...
                    eye(NbNoisRegressors(j))];

                PC_Regressor = [PC_Regressor ; ...
                    TEMP zeros(NbNoisRegressors(j),NbRegressors-size(TEMP,2))];
            end
