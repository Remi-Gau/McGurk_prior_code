...
for Subj_Ind=1:14 %length(SubjectsList)
    ...
    AnalysisFolder = fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise_3');
    ...
    for i = 1:NbRuns
        ...
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).hpf = Inf;
    ...
    cd(fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise_3'));
    
    for Day_Ind=1:2
        if exist(['DenoiseDay' num2str(Day_Ind) '.mat'], 'file') 
            load (['DenoiseDay' num2str(Day_Ind) '.mat'])
            if exist('DenoiseResults', 'var') && length(Runs2Include)>1
                for RunInd=1:length(Runs2Include)
                    Run=Runs2Include(RunInd);
                    for j=1:DenoiseResults.pcnum
                        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).regress(1,end+1).name = strcat('Noise Regresor ', num2str(j));
                        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).regress(1,end).val = double(DenoiseResults.pcregressors{1,RunInd}(:,j));
                    end
                end
            end
            clear DenoiseResults Runs2Include Run RunInd
        end
    end
    ...
    save (strcat('First_Level_Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise_3', SubjID, '_jobs'));
