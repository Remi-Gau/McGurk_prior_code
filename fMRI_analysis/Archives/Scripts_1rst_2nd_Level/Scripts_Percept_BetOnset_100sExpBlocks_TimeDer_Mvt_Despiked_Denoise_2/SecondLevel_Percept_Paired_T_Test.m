...
%%
cd SecondLevel
mkdir (fullfile(pwd, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Mvt_Despiked_Denoise_2', 'Paired_T-Test'));
cd ..

GroupAnalysisFolder = fullfile(pwd, 'SecondLevel', 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Mvt_Despiked_Denoise_2', 'Paired_T-Test');
...
for j=1:2 %length(ListOfTest)
    ...
    for i=1:length(SubjectsListTemp)
        ...
        A = fullfile(PresentFolder, num2str(SubjectsListTemp(i)), ...
            'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Mvt_Despiked_Denoise_2', ...
            'con_00');
