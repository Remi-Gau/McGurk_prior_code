...
try
    ...
    for h=1:2 %length(SubjectsList)
        
        SubjID=num2str(SubjectsList(h))

        
        SubjectFolder = strcat(PresentFolder, filesep, SubjID, filesep);
        SubjectStructuralFolder = strcat(PresentFolder, filesep, SubjID, filesep, 'Structural', filesep);
        AnalysisFolder = fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_200HPF_Despiked_Denoise');

        cd(fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise'));
