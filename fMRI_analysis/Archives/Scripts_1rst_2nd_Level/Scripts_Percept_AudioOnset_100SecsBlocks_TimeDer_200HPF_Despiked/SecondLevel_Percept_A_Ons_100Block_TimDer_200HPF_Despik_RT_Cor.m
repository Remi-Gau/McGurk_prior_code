...
cd ..
StartFolder = pwd;
GroupAnalysisFolder = strcat(StartFolder, filesep, 'SecondLevel', filesep, 'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep, 'RT_Correlation');
...
ListOfContrastOfInterests = [1 2 4 6 7 9 10];                  
...
for j=1:length(ListOfContrastNames)
    ...
    ContrastsTemp(isnan(CorrelationVectorsTemp))=[];
    YTemp(isnan(CorrelationVectorsTemp))=[]
    ...
    for i=1:length(SubjectsListTemp)
        ...
        SubjectAnalysisFolder = strcat(StartFolder, filesep, num2str(SubjID), filesep,'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep);
