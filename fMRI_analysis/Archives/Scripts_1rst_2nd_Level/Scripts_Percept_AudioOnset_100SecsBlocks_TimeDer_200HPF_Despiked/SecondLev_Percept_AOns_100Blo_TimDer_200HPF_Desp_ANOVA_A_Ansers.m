...
B = repmat([strcat(filesep, 'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep, 'con_00')], length(SubjectsList)*4, 1);
A = [A B];
clear B
...
cd Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked;
mkdir RM-ANOVA_Audio_Answers;
cd RM-ANOVA_Audio_Answers;
...
save (strcat('Second_Level_Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked_RM-ANOVA_Audio_Answers_jobs'), 'matlabbatch');
...
cname = 'McGurk in CON > McGurk in INC';
SPM.xCon(end+1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

c = -1*c;
cname = 'McGurk in INC > McGurk in CON';
SPM.xCon(end+1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

c = [ 0 1 0 -1 zeros(1,length(SubjectsList))];
cname = 'Non McGurk in CON > Non McGurk in INC';
SPM.xCon(end+1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

c = -1*c;
cname = 'Non McGurk in INC > Non McGurk in CON';
SPM.xCon(end+1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);
