...
ListOfTest = {'Blocks', 'Events_Non_McGurk', 'McGurk_Fused', 'McGurk_Auditory', 'McGurk_CON', 'McGurk_INC', 'NonMcGurk_INC'};

ContrastOfInterest = [ 12 13 ; 4 10 ; 2 7 ; 1 6 ; 1 2 ; 6 7 ; 10 9];

ListOfContrastNames = {'CON > INC', 'INC > CON' , 'CON > Baseline', 'INC > Baseline' , 'CON < Baseline', 'INC < Baseline', 'INC + CON > Baseline' , 'INC + CON < Baseline' ; ...
...
for h=1:length(SubjectsList)
        ...
        AnalysisFolder = strcat(SubjectFolder, 'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep);
...
Contrasts(end+1,:) = Contrasts(end,:)+1; Contrasts(end+1,:) = Contrasts(end,:)+1;

clear B SOT X h Sorted_SOT

cd SecondLevel
mkdir Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked;
cd ..

GroupAnalysisFolder = strcat(pwd, filesep, 'SecondLevel', filesep, 'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep);


for j=7:7%length(ListOfTest)
    ...
    if j>1
        for i=1:2
            SubjectsListTemp(Y(ContrastOfInterest(j,i),:)<=10)=[];
            Y(:,(Y(ContrastOfInterest(j,i),:)<=10))=[];
        end
    end
    ...
    for i=1:length(SubjectsListTemp)
        ...
        A = strcat(PresentFolder, filesep, num2str(SubjectsListTemp(i)), filesep, ...
            'Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked', filesep, ...
            'con_00');
