...
try
    ...
    for h=1:length(SubjectsList)
        ...
        for i=1:NbRuns
           ...
           CON_Block_SOT{1,i} = TotalTrials{3,1}(TotalTrials{1,1}(:,2)==1 & TotalTrials{1,1}(:,4)==0 & TotalTrials{1,1}(:,9)==i, :)+VideoToAudioOnsetDelay;
           INC_Block_SOT{1,i} = TotalTrials{3,1}(TotalTrials{1,1}(:,2)==1 & TotalTrials{1,1}(:,4)==1 & TotalTrials{1,1}(:,9)==i, :)+VideoToAudioOnsetDelay;
        ...
        save (strcat('First_Level_Analysis_Percept_AudioOnset_100SecsBlocks_TimeDer_200HPF_Despiked_', SubjID, '_jobs'));
