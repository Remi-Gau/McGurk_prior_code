...
for h=1:length(SubjectsList)
    ...
    cd(SubjectFolder)
    mkdir Analysis_Percept_BetweenOnset_83ExpBlocks_TimeDer_200HPF_Despiked
    ...
    for i=1:NbRuns
        ...
        Block_Durations{1,i} = [83 83 83];
        Block_Durations{2,i} = [83 83 83];
...
%             Block_Durations{1, i}(Block_Durations{1, i}==83)=[];
%         end
%         
...
%             Block_Durations{2, i}(Block_Durations{2, i}==83)=[];
%         end
...
%                             plot(sf((0:(NbVols - 1))*NbSlices + ReferenceSlice + 32,:), 'g')
    ...
    save ('SOT.mat', 'SOT', 'Sorted_SOT')
    save (strcat('First_Level_Analysis_Percept_BetweenOnset_83ExpBlocks_TimeDer_200HPF_Despiked_', SubjID, '_jobs'));
