...
ListOfContrastNames = {'McGurkInCON trials > McGurkInINC trials', 'McGurkInINC trials > McGurkInCON trials' , 'McGurkInCON trials > Baseline', 'McGurkInINC trials > Baseline' , 'McGurkInCON trials < Baseline', 'McGurkInINC trials < Baseline', 'McGurkInCON + McGurkInINC > Baseline' ; ...
                       'CON > INC', 'INC > CON' , 'CON > Baseline', 'INC > Baseline' , 'CON < Baseline', 'INC < Baseline', 'INC + CON > Baseline' ; ...
                       'CON trials > INC trials', 'INC trials > CON trials' , 'CON trials > Baseline', 'INC trials > Baseline' , 'CON trials < Baseline', 'INC trials < Baseline' , 'INC trials + CON trials > Baseline'};

SubjectsList = [1 13 14 15 24 28 32 41 48 66 69 73 74 82 98 61];

cd SecondLevel
mkdir Analysis_NoTimeDer_200HPF;
cd ..

pwd

GroupAnalysisFolder = strcat(pwd, filesep, 'SecondLevel', filesep, 'Analysis_NoTimeDer_200HPF', filesep);
...
for j=1:length(ListOfTest)
    ...
    for i=1:length(SubjectsList)
        ...
        SubjectAnalysisFolder = strcat(StartFolder, filesep, num2str(SubjID), filesep,'Analysis_NoTimeDer_200HPF', filesep);
