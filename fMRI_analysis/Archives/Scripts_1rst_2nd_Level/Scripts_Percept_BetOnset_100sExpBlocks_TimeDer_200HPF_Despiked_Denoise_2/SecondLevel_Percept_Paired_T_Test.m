...
for h=1:length(SubjectsList)
        ...
        AnalysisFolder = fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_200HPF_Despiked_Denoise_2');
...
%         {SPM.xCon(ContrastOfInterest(1,:)').name}'
%         {SPM.xCon(ContrastOfInterest(2,:)').name}'
%         {SPM.xCon(ContrastOfInterest(3,:)').name}'
%         {SPM.xCon(ContrastOfInterest(4,:)').name}'
%         {SPM.xCon(ContrastOfInterest(5,:)').name}'
...
%%
cd SecondLevel
mkdir (fullfile(pwd, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_200HPF_Despiked_Denoise_2', 'Paired_T-Test'));
cd ..

GroupAnalysisFolder = fullfile(pwd, 'SecondLevel', 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_200HPF_Despiked_Denoise_2', 'Paired_T-Test');


for j=1:2 %length(ListOfTest)
    ...
    for i=1:length(SubjectsListTemp)
        ...
        A = fullfile(PresentFolder, num2str(SubjectsListTemp(i)), ...
            'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_200HPF_Despiked_Denoise_2', ...
            'con_00');
