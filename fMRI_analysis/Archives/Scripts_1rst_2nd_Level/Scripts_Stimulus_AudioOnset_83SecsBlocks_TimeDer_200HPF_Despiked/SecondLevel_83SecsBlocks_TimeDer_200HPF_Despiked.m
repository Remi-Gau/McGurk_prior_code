...
cd SecondLevel
mkdir Analysis_83SecsBlocks_TimeDer_200HPF_Despiked;
cd ..

GroupAnalysisFolder = strcat(pwd, filesep, 'SecondLevel', filesep, 'Analysis_83SecsBlocks_TimeDer_200HPF_Despiked', filesep);
...
for j=1:length(ListOfTest)
    ...
    for i=1:length(SubjectsList)
        ...
        SubjectAnalysisFolder = strcat(StartFolder, filesep, num2str(SubjID), filesep,'Analysis_83SecsBlocks_TimeDer_200HPF_Despiked', filesep);

        matlabbatch{1,1}.spm.stats.factorial_design.des.pt.pair(1,i).scans{1,1} = strcat(SubjectAnalysisFolder, ListOfContrast{j,1});
        matlabbatch{1,1}.spm.stats.factorial_design.des.pt.pair(1,i).scans{2,1} = strcat(SubjectAnalysisFolder, ListOfContrast{j,2});
    ...
    % Estimate model
    matlabbatch{1,end+1}={};
    matlabbatch{1,end}.spm.stats.fmri_est.spmmat{1,1} = strcat(pwd, filesep, 'SPM.mat');     %set the spm file to be estimated
