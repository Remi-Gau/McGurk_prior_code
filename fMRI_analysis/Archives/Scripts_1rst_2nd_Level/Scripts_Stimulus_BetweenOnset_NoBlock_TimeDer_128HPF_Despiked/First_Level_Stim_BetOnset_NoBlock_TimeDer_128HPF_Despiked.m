...
%%
        ...
        for i = 1:NbRuns
            ...
            for j = 1:4
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).cond(1,j).name = Conditions_Names{EventsOnly(j)};
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).cond(1,j).duration = Condition_Duration{EventsOnly(j),i};
                ...
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).cond(1,j).onset = SOT{EventsOnly(j),i};
            end
        ...
        save (strcat('First_Level_Analysis_Stimulus_BetweenOnset_NoBlock_TimeDer_128HPF_Despiked_', SubjID, '_jobs'));
