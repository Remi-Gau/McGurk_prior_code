...
ListOfContrastNames = {'McGurkInCON trials > McGurkInINC trials', 'McGurkInINC trials > McGurkInCON trials' ; 'CON trials > INC trials', 'INC trials > CON trials'};

SubjectsList = [1 13 14 15 24 28 32 41 48 61 66 69 73 74 82 98];

cd SecondLevel
if exist('No_Block_Without_Time_Derivative','dir')==0
    mkdir No_Block_Without_Time_Derivative;
end;
cd ..

GroupAnalysisFolder = strcat(pwd, filesep, 'SecondLevel', filesep, 'No_Block_Without_Time_Derivative', filesep);
...
for j=1:length(ListOfTest)
    ...
    for i=1:length(SubjectsList)
        ...
        SubjectAnalysisFolder = strcat(A, filesep, num2str(SubjID), filesep,'Analysis_No_Block_Without_Temporal_Derivative', filesep);
