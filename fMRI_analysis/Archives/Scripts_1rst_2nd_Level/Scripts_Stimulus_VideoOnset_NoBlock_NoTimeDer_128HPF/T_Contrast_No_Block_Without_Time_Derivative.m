...
try
    ...
    for h=1:length(SubjectsList)
        ...
        AnalysisFolder = strcat(SubjectFolder, 'Analysis_No_Block_Without_Temporal_Derivative', filesep);
        ...
        c = [ repmat([repmat([1 1], 1, NbBlockTypes) zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        cname = 'Activation All';
        SPM.xCon = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

        c = [ repmat([repmat([1 0], 1, NbBlockTypes) zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        cname = 'McGurk Activation All';
        SPM.xCon(end + 1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

        c = [ repmat([1 0 zeros(1,2) zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        cname = 'McGurk in Congruent';
        SPM.xCon(end + 1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

        c = [ repmat([zeros(1,2) 1 0 zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        cname = 'McGurk in Incongruent';
        SPM.xCon(end + 1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs);

        c = [ repmat([1 0  -1 0  zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        ...
        c = [ repmat([0 1  0 0  zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
        cname = 'CON Trials activation';
        SPM.xCon(end + 1) = spm_FcUtil('Set', cname, 'T','c', c(:), SPM.xX.xKXs)

        c = [ repmat([0 0  0 1  zeros(1,RealParam)], 1, NbSession) zeros(1,NbSession) ];
