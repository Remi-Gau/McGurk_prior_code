...
try
    ...
    for h=1:length(SubjectsList)
        ...
        for k=1:Size_Runs_List
            ...
            SOT{2, k}= Trials{6,1}(find(Trials{1,1}(:,5)==0));
            SOT{4, k}= Trials{6,1}(find(Trials{1,1}(:,5)==1));

            for l=1:length(Trials{1,1}) 
                    if Trials{1,1}(l,5)==2
                        SOT{1 + Trials{1,1}(l,4) * 2, k} = [SOT{1 + Trials{1,1}(l,4) * 2, k} ; Trials{6,1}(l,1)];
                    end
            end
        end;
        ...
        if exist(strcat('Analysis_No_Block_Without_Temporal_Derivative'),'dir')==0
            mkdir Analysis_No_Block_Without_Temporal_Derivative;
        end;
