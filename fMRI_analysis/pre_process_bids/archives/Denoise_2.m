...
for h=3:length(SubjectsList)
    ...
    AnalysisFolder = fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise_2', filesep);
    ...
    for i=1:NbRuns
        ...
        Mov_Parameter_ls = dir('rp_*.txt');
        % Names them with its absolute pathname
        Mov_Parameter = load(fullfile(ImagesFolder, Mov_Parameter_ls.name));

        Design{1,i}=ConditionRegressors(1:size(Mov_Parameter,1),1:size(Conditions_Names,1)*2);
...
%         xlabel('Conditions');
%         ylabel('Time points');
%         title(sprintf('Design matrix (%d time points, %d conditions)',size(Design{1,i},1),size(Design{1,i},2)));
        
        ConditionRegressors(:,1:size(Conditions_Names,1)*2)=[];
        ConditionRegressors(1:size(Mov_Parameter,1),:)=[];


        
%         for j=1:size(Mov_Parameter,2)
%             opt.extraregressors{1,i}(:,end+1) = Mov_Parameter(:,j);
%         end
    ...
    save('Denoise.mat', 'Design', 'Condition_Duration', 'TR', 'opt', 'DenoiseResults')
