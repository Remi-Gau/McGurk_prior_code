...
for Subj_Ind=1:10 %length(SubjectsList)
    
    SubjID=num2str(SubjectsList(Subj_Ind))
    ...
    AnalysisFolder = fullfile(SubjectFolder, 'Analysis_Percept_BetOnset_100ExpBlock_TimeDer_Despiked_Denoise_3', filesep);
    ...
    for i=1:NbRuns
        ...
        for EmptyCondInd = 1:sum(EmptyCond)
            if ~isempty(Sorted_SOT{1,12})
                Sorted_SOT{1,find(EmptyCond,1)} = Sorted_SOT{1,12}(1);
                Sorted_SOT{1,12}(1) = [];
            else
                CondMax = max(cellfun('length', Sorted_SOT));
                Sorted_SOT{1,find(EmptyCond,1)} = Sorted_SOT{1,find(CondMax==cellfun('length', Sorted_SOT),1)}(1);
                Sorted_SOT{1,find(CondMax==cellfun('length', Sorted_SOT),1)}(1)=[];
            end
            EmptyCond = cellfun('isempty', Sorted_SOT);
        end
        ...
        if isempty(Sorted_SOT{1,12})
            CondMax = max(cellfun('length', Sorted_SOT));
            Sorted_SOT{1,12}(1) = Sorted_SOT{1,find(CondMax==cellfun('length', Sorted_SOT),1)}(1);
            Sorted_SOT{1,find(CondMax==cellfun('length', Sorted_SOT),1)}(1)=[];
        end
        
        % Input conditions in SPM batch
        Counter = 1;
        for j = 1:size(Conditions_Names,1)
            if isempty(Sorted_SOT{1,j})
            else
                ...
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).cond(1,Counter).onset = Sorted_SOT{1,j};
        ...
        % Create onset vectors and convolve them
        for j=1:2
...
%     delete *.*
    ...
    save (strcat('First_Level_Analysis_Percept_BetOnset_100ExpBlocks_TimeDer_Despiked_Denoised', SubjID, '_jobs'));
...
%     spm_jobman('run', matlabbatch)
    ...
    ConditionRegressors = ~cellfun('isempty', strfind(SPM.xX.name','bf('));
    ConditionRegressors = SPM.xX.X(:,ConditionRegressors);
    
    Session = [];
    
    for Run_Ind=1:NbRuns
        Session(:,:,Run_Ind) = ConditionRegressors(1:NbVols,1:size(Conditions_Names,1)*2);
        ConditionRegressors(:,1:size(Conditions_Names,1)*2)=[];
        ConditionRegressors(1:NbVols,:)=[];
    end

    
    for Day_Ind = 1:size(RunPerDay,2)
        
        Design = {};
        
        Runs2Include = RunPerDay{Subj_Ind,Day_Ind};
        
        DenoiseFolder = ['DenoiseFigDay' num2str(Day_Ind)];
        mkdir(DenoiseFolder)
        
        if ~isempty(Runs2Include)
            for Run_Ind=1:length(Runs2Include)
                
                Run=Runs2Include(Run_Ind);
                
                %% Design
                Design{1,Run_Ind}=Session(:,:,Run);

                opt.extraregressors{1,Run_Ind} = BlockRegressor{1,Run};
    
                %% DATA
                fprintf('Collects data.\n')
                % Enter source folder reads the image files
                ImagesFolder = fullfile(NiftiSourceFolder, num2str(Run));
                cd (ImagesFolder)
                ...
                Data{1,Run_Ind} = single(spm_read_vols(spm_vol(char(ImageList))));
            ...
            %% Denoise
            cd(AnalysisFolder)
            save(['DenoiseDay' num2str(Day_Ind) '.mat'], 'Design', 'Condition_Duration', 'TR', 'opt', 'Runs2Include')
