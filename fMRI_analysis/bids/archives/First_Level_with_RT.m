...
try
    ...
    for h=1:length(SubjectsList)
        ...
        cd(SubjectFolder)
        mkdir Analysis_Percept_RT_BetweenOnset_100ExpBlocks_TimeDer_200HPF_Despiked
        ...
        % Gets the reations times
        A = TotalTrials{1,1}(:,[6 8 9]);
        B = median(A(A(:,2)~=5,1));
        A(A(:,2)==5,1) = B;
        for Run = 1:NbRuns
            ReactionTimes{Run} = A(A(:,3)==Run,1);
            Pooled_SOT(:,Run) = sort(cell2mat({Sorted_SOT{:,Run}}'));
        end
        
        cd (AnalysisFolder)
        
        save ('SOT.mat', 'SOT', 'Sorted_SOT', 'ReactionTimes')
        
        fprintf('\nSOTs collected.\n\n')
        
        
        
        %% Creates a dummy design matrix to get the RT regressor
        matlabbatch = {};
        ...
        for Run = 1:NbRuns
            ...
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).regress = struct('name',{},'val',{});
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).hpf = 200;                   
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).multi_reg{1} = '';
            ...
            for j = 1:NbVols
                matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).scans{j,1} = [ImagesFolder, filesep, IMAGES_ls(j).name];
            end


            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).name = 'Trials';
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).duration = Condition_Duration;
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).tmod = 0;

            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).pmod.name = 'Reaction Times';
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).pmod.param = ReactionTimes{Run};
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).pmod.poly=1;

            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,Run).cond(1,1).onset = Pooled_SOT(:,Run);
        ...
        A = SPM.xX.X(:,2:2:NbRuns*2);
        B = [1+((1:NbRuns)-1)*NbVols ; (1:NbRuns)*NbVols]
        B = reshape(B,1,size(B,1)*size(B,2));
        if NbRuns < 8
        ReactionTimesParamMod = [ A(B(1):B(2), 1) ...
                                  A(B(3):B(4), 1) ...
                                  A(B(5):B(6), 1) ...
                                  A(B(7):B(8), 1) ...
                                  A(B(9):B(10), 1) ...
...
%                 plot(sf((0:(NbVols - 1))*NbSlices + ReferenceSlice + 32,:), 'g')
            ...
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).regress(1,Counter).name = 'Reaction time parametric modulator';
            matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,i).regress(1,Counter).val = ReactionTimesParamMod(:,i);
